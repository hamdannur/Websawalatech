...
const services = [
  {
    icon: <Code2 size={60} className="text-blue-600" />,
  ...
  {
    icon: <Cloud size={60} className="text-blue-600" />,
  ...
  {
    icon: <Sparkles size={60} className="text-blue-600" />,
...
const ServiceCard = ({ svc }: { svc: typeof services[0] }) => (
  <div className="flex flex-col gap-4 w-[340px]">
    ...
    <div className="flex flex-col gap-4">
      <h3 className="text-[#1f2937] text-[24px] leading-normal whitespace-pre-wrap" style={{ fontFamily: "'Inter', sans-serif", fontWeight: 500 }}>
        {svc.title}
      </h3>
      <p className="text-[#4b5563] text-[16px] leading-[24px]" style={{ fontFamily: "'Inter', sans-serif", fontWeight: 400 }}>
        {svc.desc}
      </p>
      <ul className="flex flex-col gap-3">
        {svc.items.map((item, j) => (
          <li key={j} className="flex items-center gap-3">
            ...
            <span className="text-[#4b5563] text-[16px] leading-[24px]" style={{ fontFamily: "'Inter', sans-serif", fontWeight: 400 }}>{item}</span>
          </li>
        ))}
      </ul>
    </div>
...
export function ServicesSection() {
  return (
    <>
      {/* ── Mobile ── */}
      <section className="md:hidden w-full bg-white px-[16px] py-[70px]">
        <div className="flex flex-col gap-[60px] items-center">
          <h2 className="text-black text-center leading-[1.3] tracking-[-1.2px] w-full" style={{ fontFamily: "'Inter', sans-serif", fontWeight: 500, fontSize: 30 }}>
      ...
      {/* ── Tablet ── */}
      <section className="hidden md:block lg:hidden w-full bg-white px-[32px] py-[70px]">
        <div className="flex flex-col gap-[60px] items-center">
          <h2 className="text-black text-center leading-[1.3] tracking-[-1.2px] w-full" style={{ fontFamily: "'Inter', sans-serif", fontWeight: 500, fontSize: 30 }}>
            We take care of your technology, so you can focus on growing your business
          </h2>
          <div className="flex flex-wrap gap-[54px_24px] items-start">
            {services.map((svc, i) => <ServiceCard key={i} svc={svc} />)}
          </div>
        </div>
      </section>
      ...
      {/* ── Desktop ── */}
      <section className="hidden lg:block w-full bg-white py-[70px] px-[100px] overflow-x-clip">
        <div className="max-w-[1440px] mx-auto flex flex-col gap-[60px]">
          <div className="flex justify-center">
            <h2 className="text-black text-center text-[48px] leading-[72px] tracking-[-1.92px] max-w-[933px]" style={{ fontFamily: "'Inter', sans-serif", fontWeight: 500 }}>
              We take care of your technology, so you can focus on growing your business
            </h2>
          </div>
          <div className="flex gap-[120px] items-start justify-center">
            ...
            <div className="hidden xl:block self-stretch">
              <div className="flex flex-wrap gap-[26px] items-start justify-end pt-5 max-w-[608px] sticky top-[110px] z-10">
